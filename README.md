...
| 能力 | 行为 | 如何操作 |
| --- | --- | --- |
| 执行意图与指派分离 | 手动或本机 `assign` 只保存账号并强制 `hold`，绝不 arm/调度/触发页面动作 | UI：任务卡选择账号即完成指派；启动须再点「启动」。本机：`POST .../assign` 与 `POST .../start` 必须是两个独立 `requestId` |
| 显式启动门禁 | 只有单任务启动、批量 arm、CSV 导入后执行、本机 `start` 或 `retry` 才会置 `armed`；`processQueue` 只处理 `armed` | 不要把「已指派」当成「已授权执行」；历史缺失意图按 hold 迁移，不自动补跑 |
| 原会话定位 | 任务卡双击或「核对平台结果」只导航/刷新该任务绑定的原会话，绝不新建会话或重新发送 | 双击任务卡切换到正确账号并打开原会话；定位信息缺失时 fail-closed，需人工在网页处理 |
| 本机账号/产物接口 | `GET /v1/accounts`、`GET .../artifacts` 返回脱敏投影；`POST .../artifacts/{id}/download` 按稳定 artifact ID 下载 | 先 `ready=true`，再用返回的稳定 ID 调用；调用方不能传 URL 或目录。详见 [`docs/LOCAL_CONTROL.md`](docs/LOCAL_CONTROL.md) |
| 依赖阻断可恢复 | 阻断保持 `queued` + `blockedByTaskIds`，不写 `fail`；前置恢复后自动清阻断并只启动一次 | 界面看「依赖阻断」标签；无需手工重试后继。依赖错误不计平台失败率/额度/健康冷却 |
| 精确批量下载 | 必须单项目、单批次、非空产物；缺归属/零产物/旧裸数组事件整体拒绝 | 先选批次再打开预览；默认全不选；按钮显示项目/批次/任务/产物计数，无效项会禁用下载 |
| 人工质量裁决 | 非法质量标签整体拒绝；非法或缺失 `decidedAt` 整体丢弃，不补当前时间 | 在任务详情按合法标签裁决；历史非法时间不会被“修成现在” |

> [!WARNING]
> Dola 仍未完成真实端到端验收。本机指派/下载对未验证平台返回 `PLATFORM_NOT_VERIFIED`，不得写成生产可用。
...
- 每次打开批量下载预览都从**全不选**开始，不会沿用上一次的选择。
- 下载范围固定为**当前项目 + 用户明确选择的单个批次**；没有当前批次时先选择批次，绝不退化为全部历史任务或跨项目产物。
- 当前版本**不支持跨批次下载**：预览中默认只会出现同一项目同一批次内的候选；任何缺少项目或批次归属、或没有可下载产物的选择都会被整体拒绝，不发起下载调用。
- 下载按钮和摘要实时显示**当前选择**的项目数、批次数、任务数和产物数；选择为空或选择包含无效项时按钮禁用。
- 单任务、单产物下载保持原有行为。

### 依赖阻断语义

- 前置任务失败/取消、依赖 ID 不存在、依赖成环分别写入 `dependency_failed`、`dependency_missing`、`dependency_cycle`，并把原因任务 ID 持久化到 `blockedByTaskIds`。
- 依赖阻断是**调度阻断**，不是平台生成失败：它不计入运行统计中的平台成功率/失败率，不增加账号连续失败、健康冷却或额度消耗，也不会进入批次“重试失败”或自动重试。
- 被阻断的任务**保持排队状态并留在调度队列中**，界面用独立的「依赖阻断」标签展示。依赖恢复后调度器会自动重新评估、清除阻断证据并放行，不需要人工重试；即使应用重启，也会从盘面恢复阻断证据并继续等待重新评估。
- 写入不会伪报成功：写盘确实失败时界面保持写入前的原状态；写盘已发生但不满足请求时，服务端返回权威回读状态与并发冲突原因，界面以落盘事实为准。
...
5. **准备新版本 Release 时**，必须先在 `CHANGELOG.md` 写清该版本完成了什么更新；CI 发布工作流会提取对应章节写入 GitHub Release 正文，缺失时会失败。
...
- 在现有 `--local-control` 稳定协议上继续收敛事件订阅与 Agent 客户端，而不是另起坐标式远程 API。
- 完成源码版本、人工验收、安装包和 GitHub Release 的独立收口；每个新 Release 必须附带更新说明。
