...
**禁止选项**（Schema `additionalProperties: false` 保证）：
...
**禁止字段**（Schema `additionalProperties: false` 保证）：
...
**输出**：`TaskSnapshot`（`status` 为 `cancelled` 或 `pausing`→`cancelled`）
...
| `executing` / `generating` | 发送中断信号，等待当前步骤安全终止 | `cancelled`（异步完成） |
...
| `tasks_events` | `tasks events` | `{ since?: integer, taskId?: string }` | `TaskEvent[]` |
...
{
  "name": "tasks_events",
  "description": "拉取任务事件流。使用 since 参数实现断线续订：发送上次收到的 lastSequence，返回所有 sequence > lastSequence 的事件。至少一次投递语义下，客户端通过 eventId 去重。",
  "inputSchema": {
    "type": "object",
    "additionalProperties": false,
    "properties": {
...
- 正在执行中的任务取消为异步操作：`tasks_cancel` 返回时 status 可能为 `pausing`，后续通过事件收到 `cancelled`
...
2. `tasks create` 的 Schema 已通过 `additionalProperties: false` 禁止客户端指定 `accountId`，服务端自动调度。
...
**G-303 决策**：**确认当前设计，补充客户端应对策略**。

客户端策略：

1. **正常续订**：客户端记录 `lastSequence`，每次调用 `tasks_events --since <lastSequence>`。
2. **检测重启**：客户端发现返回的 sequence 小于 `lastSequence` 时，判定服务端已重启。
3. **重启恢复**：
   - 客户端清空本地 `lastSequence`，从 0 重新同步
   - 通过 `eventId` 集合去重已处理过的事件
   - 如果客户端没有保留 `eventId` 集合，则接受可能的重复事件
4. **间隙容忍**：sequence 不保证连续（可能有间隙），客户端只关注 `sequence > since` 的事件。

**CLI `--follow` 模式的重启处理**：
- `--follow` 模式下 CLI 内部维护 `lastSequence` 和 `eventId` 集合
- 检测到 sequence 回退时，输出警告 `# Warning: sequence reset detected, re-synchronizing from 0`，然后从 0 重新拉取
...
- **Token 生成**：桌面实例首次启动时生成随机 Token，存储在 `userData` 目录下的配置文件中，权限 0600。
...
| Cookie / Session 泄露 | Agent 获取豆包账号凭证 | Schema `additionalProperties: false` 禁止返回 Cookie/Session 字段；CLI/MCP 映射不暴露 `partition` |
...
| Token 泄露 | 未授权方调用本地 API | Token 存储权限 0600；默认只监听 localhost；远程访问需额外 TLS 和认证 |
