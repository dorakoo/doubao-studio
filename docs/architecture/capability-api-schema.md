...
4. **开放枚举**：所有枚举字段使用开放字符串类型而非封闭 `enum`，description 中列出已知值，客户端必须处理未知值。
...
1. **幂等键**：`requestId` 由客户端生成（建议 UUID v4）。相同 `requestId` 在**当前服务进程生命周期内**重复提交时返回同一任务，不创建新任务。进程重启后 requestId 缓存丢失；响应丢失时客户端必须先通过 requestId 查询端点核对任务，再决定是否重试。
...
### 3.2.1 requestId 查询与恢复

为处理“提交成功但响应丢失”的场景，G-303 必须实现受控查询端点：

```text
GET /tasks:lookup?requestId={requestId}
```

- 查询键为客户端原始 `requestId`，不要求客户端事先知道 `taskId`。
- 当前进程仍持有映射时返回 `{ found: true, taskId, snapshot }`。
- 映射不存在时返回 `{ found: false }`，不得把“未找到”伪装成任务失败。
- 进程重启后映射可能丢失；此时客户端只能获得 `found: false`，不得据此断言原任务未执行。
- 客户端只有在查询确认 `found: false` 且接受重复风险时，才能重新提交；服务端和 CLI/MCP 映射必须记录该语义。
...
- **稳定错误码**：`code` 为开放字符串，description 中列出已知错误码，不改变已有码的语义，客户端必须处理未知值
...
v1 Schema 中**不使用封闭 `enum`**。所有概念上为枚举的字段均定义为 `type: string`，并在 `description` 中以"已知值"前缀列出当前支持的值集合。这确保严格校验器不会拒绝未来新增值。
...
- **客户端重试建议**：响应丢失或进程重启后，客户端应先使用 `GET /tasks:lookup?requestId={requestId}` 查询；只有在明确得到 `found: false` 且接受进程重启造成的重复风险时才重新提交
- 不承诺跨进程重启的幂等性
