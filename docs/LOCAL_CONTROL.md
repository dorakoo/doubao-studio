...
豆包工作室 **2.3.8** 的受控接口用于让本机 Agent 可靠控制**正在运行的正式实例**。它按项目、批次和任务 ID 定位，不依赖截图坐标、DOM 序号或跨会话元素编号。2.3.5 修复首次就绪竞态；2.3.6 确保后台账号 Webview 预热不会撤销主 Renderer 的控制桥就绪状态；2.3.7 增加视频页面分阶段就绪与受理后持久化观察；**2.3.8** 将执行意图与指派彻底分离，并补齐脱敏账号查询、只指派、产物查询与按稳定 artifact ID 的单产物下载。

| 入口 | 用途 | 是否可用于正式任务 | 生命周期 |
| --- | --- | --- | --- |
| `--local-control` | 项目、批次、任务、账号与产物的脱敏查询，以及只指派（`assign`，永远 hold）、执行控制（`start/pause/cancel/retry`）和单产物下载 | 是；必须 Bearer 鉴权并先确认 `ready=true` | 随正式实例运行，最长受 8 小时令牌期限约束 |
